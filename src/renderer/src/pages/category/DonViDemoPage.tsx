...
export default function DonViDemoPage({
  ...
  // Entity type for history
  const historyEntity = useMemo(() => {
    if (title === 'Phòng ban') return 'phongban'
    if (title === 'Trung tâm') return 'trungtam'
    if (isKhoaMode || title.startsWith('Khoa -')) return 'khoa'
    return 'donvi'
  }, [title, isKhoaMode])
    ...
    <div className="flex flex-col w-full h-full overflow-hidden relative bg-white">
      ...
      <div className="px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-3 bg-white border-b border-gray-100">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {canCreate && (
            <HrPrimaryButton startContent={<Plus size={18} />} className="px-4" onPress={() => { setFormData({}); onOpenDrawerAdd() }}>
              Thêm mới
            </HrPrimaryButton>
          )}
        ...
        {isKhoaMode ? (
          ...
          <>
            <ModalCommon
              ...
              handleSubmitApi={(_id, data) => KhoaAxios.create({ ...Object.fromEntries(data!), id_truong: idTruong })}
              formData={formData}
            ...
            <ModalCommon
              ...
              handleSubmitApi={(_id, data) => KhoaAxios.update(String(editingId), Object.fromEntries(data!))}
              formData={formData}
              onSubmitSuccess={() => { refetchKhoa(); setFormData({}) }}
            >
          ...
          <>
            <ModalCommon
              ...
              open={isOpenDrawerAdd}
              onClose={() => { onCloseDrawerAdd(); setFormData({}) }}
              handleSubmitApi={(_id, data) => apiService.create(Object.fromEntries(data!))}
              formData={formData}
            ...
            <ModalCommon
              ...
              open={isOpenDrawerEdit}
              onClose={() => { onCloseDrawerEdit(); setFormData({}) }}
              handleSubmitApi={(_id, data) => apiService.update(String(editingId), Object.fromEntries(data!))}
              formData={formData}
              onSubmitSuccess={() => { refetch(); setFormData({}) }}
            >
      ...
      <ConfirmModal
        ...
        isDanger={false}
      />
      <HistoryDrawer isOpen={isHistoryOpen} onClose={() => setIsHistoryOpen(false)} entityType={historyEntity} />
    </div>
  )
}
