...
function createMockService<T extends Record<string, any>>(
  ...
  return {
    ...
    create(payload: any) {
      const id = nextId++
      const newItem: any = { ...(opts?.mapPayload ? opts.mapPayload(payload) : payload), [pk]: id }
    ...
    update(id: string | number, payload: any) {
      ...
      if (idx >= 0) {
        ...
        if (payload?.inline_edit) {
          data[idx] = { ...data[idx], [payload.column]: payload.value }
        } else {
          data[idx] = { ...data[idx], ...payload, [pk]: data[idx][pk] }
        }

        const diffs = computeDiffs(oldItem, payload, pk, payload?.inline_edit)
