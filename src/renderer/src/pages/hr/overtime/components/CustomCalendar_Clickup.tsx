...
/* ═══════════════════════════════
   Constants
═══════════════════════════════ */
const WEEKDAY_LABELS_GOOGLE = ['Chủ nhật', 'Thứ 2', 'Thứ 3', 'Thứ 4', 'Thứ 5', 'Thứ 6', 'Thứ 7']
...
const EventCard = ({
  ...
  return (
    <button
      ...
      className={`event-card group w-full ${!isCompact ? 'h-full flex flex-col items-start justify-start' : 'block'} text-left rounded-md transition-all duration-150 hover:shadow-md cursor-pointer overflow-hidden`}
      style={{
    ...
    >
      {isCompact ? (
        <div className="flex items-center gap-1.5 px-2 py-2">
          ...
          <span className="text-xs font-bold leading-none truncate">{event.title}</span>
        </div>
      ) : (
        ...
        {days.map((day, idx) => {
          ...
          return (
            <div
              ...
              `}
            >
              {/* Day Name (Only on first row, Top Left) */}
              {isFirstRow && (
                <div className="absolute top-1.5 left-2 text-[11px] font-medium text-[#70757a] uppercase tracking-wider select-none z-10">
                  {weekdayLabels[idx]}
                </div>
              )}

              {/* Events Area (Middle, below label if first row) */}
              <div className={`flex-1 flex flex-col gap-[2px] px-1 min-h-0 overflow-y-auto custom-scrollbar ${isFirstRow ? 'mt-8' : 'mt-1'}`}>
                ...
                {hasMore && (
                  <button
                    type="button"
                    className="text-[10px] font-medium text-[#1a73e8] hover:bg-[#e8f0fe] rounded px-1 py-px text-left transition-colors mt-1"
                    onClick={(e) => {
                      e.stopPropagation()
                      onDateClick?.(dateStr)
                    }}
              ...
              <div className="flex justify-end p-1.5 mt-auto shrink-0 z-10">
                <span
                  className={`
                    inline-flex items-center justify-center min-w-[24px] h-[24px] px-1 text-[12px] rounded-md leading-none select-none
                    ${isToday
                      ? 'bg-[#1a73e8] text-white font-bold'
                      : isCurrentMonth
                        ? 'text-[#444746] font-medium'
                        : 'text-[#8c9094] font-medium'
                    }
                  `}
                >
                  {day.date() === 1 && !isToday ? `Thg ${day.month() + 1} ${day.date()}` : day.date()}
                </span>
              </div>
            </div>
          )
